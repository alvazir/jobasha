...
pub(crate) fn write_output_plugin(
    ...
    if cfg.dry_run {
        ...
        let text =
            "Everything is great right now, there is nothing to place into the output plugin";
        msg(text, MsgTone::Good, 0, cfg, log)?;
        return Ok(());
    }
    if cfg.output.dir_path != Path::new("") && !cfg.output.dir_path.exists() {
        create_dir_all(&cfg.output.dir_path).with_context(|| {
            format!(
                "Failed to create output plugin directory \"{}\"",
                cfg.output.dir_path.display()
            )
        })?;
        let text = format!(
    ...
    plugin.save_path(&cfg.output.path).with_context(|| {
        format!(
            "Failed to write output plugin to \"{}\"",
            cfg.output.path.display()
        )
    })?;
    let text = format!(
        "Output plugin was written to \"{}\"",
        cfg.output.path.display()
    );
    msg(text, MsgTone::Good, 0, cfg, log)?;
    Ok(())
